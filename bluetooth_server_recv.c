...
int main()
{
    ...
    while (1)
    {
        ...
        char buffer[1024] = { 0 };
        FILE *fp = fopen("/home/pi/AI/TempCSV/inValue.txt", "w");
        if ( recv_message == NULL ) 
        {
            fwrite("powerOff", 8, 1, fp);
        ...
        {            
            fwrite(recv_message, strlen(recv_message), 1, fp);
            fflush(fp);
            fclose(fp);
            fp = fopen("/home/pi/AI/TempCSV/outValue.txt", "r");
            fread(buffer, sizeof(buffer), 1, fp);
            write_server(client, buffer);
            fclose(fp);
        }
    }
}
