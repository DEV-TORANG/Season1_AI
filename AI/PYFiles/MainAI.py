...
def AutoMode(Firsttime, heatFeelLevel, filename):
    # 내부 온도(온습도), 내부 습도(온습도)
    ...
    WriteAndReadTXT.WriteforAndroid_auto(pressValue)
...
# 오토 모드를 껐을때 실행되는 함수.
def PassiveMode(pressValue, filename):
...
while startValue != 1:
    if startValue == 1 :
        break
    powerOnOff, CheckAuto, a, b, c = WriteAndReadTXT.ReadforOS()
...
WriteAndReadTXT.WriteforAndroid()
...
while powerOnOff != 0 :
    ...
    if powerOnOff == 1:
        ...
        if CheckAuto == 1:
            print("Auto Start")
            AutoMode(Firsttime, heatFeelLevel, filename)
        ...
        # 오토 모드가 아니라면, 실행. (수동 모드)
            ...
            print("Passive Fin")
            PassiveMode(int(pressValue), filename)
            Cnt += 1
            print("Cnt :: ",Cnt)
